...
let sourcesToLoad = document.querySelectorAll("source[data-src]")
...
const loadSources = (source) => { //Make a function to put data-src into src
...
if ("IntersectionObserver" in window) { //Is there an intersetion observer class?
    const sourceobserver = new IntersectionObserver((items, observer) => { //Make an instance
        items.forEach((item) => { //Check each triggering item
            if (item.isIntersecting) { //Check if it's intersecting...the page
                loadSources(item.target); //See above
                sourceobserver.unobserve(item.target); //Turn off the observer
            }
        });
    });
    sourcesToLoad.forEach((source) => { //Set the observer to observe all the images
        sourceobserver.observe(source);
    });
} else { //No, there is not an intersection observerer so...
    sourcesToLoad.forEach((source) => {  // load them all
        loadSources(source);
    });
}
